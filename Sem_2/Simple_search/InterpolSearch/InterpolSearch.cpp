...
int interpolSearchRecursive(int arr[], int low, int high, int x) {
	...
	int pos = low + ((x - arr[low]) * (high - low)) / (arr[high] / arr[low]);
	if (arr[pos] == x) return pos;
	if (arr[pos] < x) interpolSearchRecursive(arr, pos+1, high, x);
	return interpolSearchRecursive(arr, low, pos - 1, x);
}
...
int main() {
	...
	int resr = interpolSearchRecursive(Arr, 0, n - 1, x);
